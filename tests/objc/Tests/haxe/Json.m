...
+ (NSMutableString*) stringify:(id)value{
	return [[[Json alloc] init] toString:value];
}
@synthesize buf;
@synthesize str;
@synthesize pos;
	...
	switch ( $e.index ) {
		...
		var MATCH _g_eTClass_0 : Class = $e.params[0]{
			if (_g_eTClass_0 == NSMutableString) [self quote:v];
				...
				int len = v1.length;
				if (len > 0) {
					[self toStringRec:[v1 objectAtIndex:0]];
					int i = 1;
					while (i < len) {
						[self.buf.b appendString:(NSMutableString*)@","];
						[self toStringRec:[v1 objectAtIndex:i++]];
					}
				}
				[self.buf.b appendString:(NSMutableString*)@"]"];
			}
