...
+ (NSMutableString*) __format_get:(NSDate*)d e:(NSMutableString*)e{
	return - (NSMutableString*) switch (e){
		case @"%":{
			$r = @"%"}break;
		case @"C":{
			$r = [StringTools lpad:[Std string:[Std int:[dgetFullYear] / 100]] c:@"0" l:2]}break;
		case @"d":{
			$r = [StringTools lpad:[Std string:[dgetDate]] c:@"0" l:2]}break;
		case @"D":{
			$r = [DateTools __format:d f:@"%m/%d/%y"]}break;
		case @"e":{
			$r = [Std string:[dgetDate]]}break;
		case @"H":case @"k":{
			$r = [StringTools lpad:[Std string:[dgetHours]] c:( (e == @"H") ? @"0" : @" ") l:2]}break;
		case @"I":case @"l":{
			$r = - (NSMutableString*) int hour = [dgetHours] % 12
		...
		case @"m":{
			$r = [StringTools lpad:[Std string:[dgetMonth] + 1] c:@"0" l:2]}break;
		case @"M":{
			$r = [StringTools lpad:[Std string:[dgetMinutes]] c:@"0" l:2]}break;
		case @"n":{
			$r = @"\n"}break;
		case @"p":{
			$r = ( ([dgetHours] > 11) ? @"PM" : @"AM")}break;
		case @"r":{
			$r = [DateTools __format:d f:@"%I:%M:%S %p"]}break;
		case @"R":{
			$r = [DateTools __format:d f:@"%H:%M"]}break;
		case @"s":{
			$r = [Std string:[Std int:[dgetTime] / 1000]]}break;
		case @"S":{
			$r = [StringTools lpad:[Std string:[dgetSeconds]] c:@"0" l:2]}break;
		case @"t":{
			$r = @"\t"}break;
		case @"T":{
			$r = [DateTools __format:d f:@"%H:%M:%S"]}break;
		case @"u":{
			$r = - (NSMutableString*) int t = [dgetDay]
		...
		case @"w":{
			$r = [Std string:[dgetDay]]}break;
		case @"y":{
			$r = [StringTools lpad:[Std string:[dgetFullYear] % 100] c:@"0" l:2]}break;
		case @"Y":{
			$r = [Std string:[dgetFullYear]]}break;
		default:{
