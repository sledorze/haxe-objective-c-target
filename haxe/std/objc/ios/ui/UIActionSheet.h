...
- (id)initWithTitle:(NSString *)title delegate:(id<UIActionSheetDelegate>)delegate cancelButtonTitle:(NSString *)cancelButtonTitle destructiveButtonTitle:(NSString *)destructiveButtonTitle otherButtonTitles:(NSString *)otherButtonTitles, ... NS_REQUIRES_NIL_TERMINATION;
	...
	public var  id<UIActionSheetDelegate> delegate;    // weak reference
public var  NSString *title;
	public var  UIActionSheetStyle actionSheetStyle; // default is UIActionSheetStyleAutomatic. ignored if alert is visible
...
- (NSString *)buttonTitleAtIndex:(NSInteger)buttonIndex;
	public var (default, null) NSInteger numberOfButtons;
	public var  NSInteger cancelButtonIndex;      // if the delegate does not implement -actionSheetCancel:, we pretend this button was clicked on. default is -1
	public var  NSInteger destructiveButtonIndex;        // sets destructive (red) button. -1 means none set. default is -1. ignored if only one button

	public var (default, null) NSInteger firstOtherButtonIndex;	// -1 if no otherButtonTitles or initWithTitle:... not used
...
// UIActionSheetStyleDefault if automatic style set
	public function showFromToolbar:(UIToolbar *)view;
	public function showFromTabBar:(UITabBar *)view;
	public function showFromBarButtonItem:(UIBarButtonItem *)item animated:(BOOL)animated NS_AVAILABLE_IOS(3_2);
	public function showFromRect:(CGRect)rect inView:(UIView *)view animated:(BOOL)animated NS_AVAILABLE_IOS(3_2);
	public function showInView:(UIView *)view;

// hides alert sheet or popup. use this method when you need to explicitly dismiss the alert.
// it does not need to be called if the user presses on a button
	public function dismissWithClickedButtonIndex:(NSInteger)buttonIndex animated:(BOOL)animated;

}


extern interface UIActionSheetDelegate <NSObject>
@optional

// Called when a button is clicked. The view will be automatically dismissed after this call returns
	public function actionSheet:(UIActionSheet *)actionSheet clickedButtonAtIndex:(NSInteger)buttonIndex;

// Called when we cancel a view (eg. the user clicks the Home button). This is not called when the user clicks the cancel button.
// If not defined in the delegate, we simulate a click in the cancel button
	public function actionSheetCancel:(UIActionSheet *)actionSheet;

	public function willPresentActionSheet:(UIActionSheet *)actionSheet;  // before animation and showing view
	public function didPresentActionSheet:(UIActionSheet *)actionSheet;  // after animation

	public function actionSheet:(UIActionSheet *)actionSheet willDismissWithButtonIndex:(NSInteger)buttonIndex; // before animation and hiding view
	public function actionSheet:(UIActionSheet *)actionSheet didDismissWithButtonIndex:(NSInteger)buttonIndex;  // after animation
